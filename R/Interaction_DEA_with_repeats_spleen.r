...
# Export raw counts matrix from the last repeat
raw_counts <- counts(dds, normalized=FALSE)
write.csv(raw_counts, "res_raw_counts.csv")

# Export normalized counts matrix
norm_counts <- counts(dds, normalized=TRUE)
write.csv(norm_counts, "res_normalized_counts.csv")

# Export gene information (rowData)
gene_info <- as.data.frame(rowData(dds))
write.csv(gene_info, "res_gene_info.csv")

# Export sample information (colData)
sample_info <- as.data.frame(colData(dds))
write.csv(sample_info, "res_sample_info.csv")
